...
"""
code optimization in leave-one-out
changed by Chen Wang
2019.11.23
"""
...
def init_evaluate_model(model, dataset, args):
    ...
    for idx in range(len(dataset.testRatings)):
        ...
        items = list(range(dataset.num_items))  # rank on all items, TODO(wgcn96): 不需要在尾部添加 ground truth item
        # items.append(gtItem)
...
# for evaluating the saved model
...
def eval(model, sess, dataset, DictList, args, behave_type=None):
    ...
    if args.model == 'FISM':
        for idx in range(len(dataset.testRatings)):
            t1 = time()
            user, gtItem = dataset.testRatings[idx]
            items = list(range(dataset.num_items))  # rank on all items
            items.append(gtItem)
            user_input = np.full(len(items), user, dtype='int32')[:, None]
            item_input = np.array(items)[:, None]
            # item rate / item_num 
...
def _eval_one_rating(idx, behave_type):
    ...
    if _model_name in ['Multi_GMF', 'Multi_MLP', 'Multi_NCF', 'CMF'] or 'Our' in _model_name:
    ...
    #
    # if idx < 3:
    #     print('idx %d\n all predictions %s\n item %s, rank %s, rank_score %s' % (idx, predictions[:50], gtItem, rank, rank_score))
    #     print('sort_idx %s' % sort_indexes[:50])
    #
    # if rank < _K:
    #     hr = 1
    #     ndcg = math.log(2) / math.log(rank + 2)
