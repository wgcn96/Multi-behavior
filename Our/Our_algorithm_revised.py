...
class Our_Algorithm_WtNCF():
    ...
    def _create_variables(self):
        ...
        with tf.name_scope('attention_layer'):      # TODO(wgcn96): wt NCF but still should have H
            pass
...
class Our_Algorithm_MLP():
    ...
    def _create_variables(self):
        ...
        with tf.name_scope('NCF'):
            ...
            # the h-vector in original paper
            v_size = int(self.embedding_size / (2 ** (self.layer_num - 1)))
            self.v_1 = tf.Variable(tf.random_uniform([v_size, 1], minval=-tf.sqrt(3 / v_size),
                                                     maxval=tf.sqrt(3 / v_size)), name='v_1')
            self.v_2 = tf.Variable(tf.random_uniform([v_size, 1], minval=-tf.sqrt(3 / v_size),
                                                     maxval=tf.sqrt(3 / v_size)), name='v_2')
    ...
    def _create_inference(self):
        ...
        with tf.name_scope('inference'):
            ...
            if self.layer_num == 0:
                ...
                if self.b_num == 3:
                    # predict ipv
                    latent_factor_1 = tf.nn.relu(tf.matmul(concat_vec_1, self.W1) + self.b1, name='latent_factor_1')
                    output_ipv = tf.matmul(latent_factor_1, self.v_1)
                ...
                if self.b_num == 3:
                    # predict ipv
                    output_ipv = tf.matmul(fc_1[i], self.v_1, name='latent_factor_1')

                    # predict cart
                    output_cart = tf.matmul(fc_2[i], self.v_2, name='latent_factor_2')

                    # predict buy
                    output_buy = tf.matmul(fc_3[i], self.v_3, name='latent_factor_3')
...
class Our_Algorithm_GMF():
    ...
    def _create_inference(self):
        ...
        with tf.name_scope('inference'):
            ...
            if self.b_num == 3:
                # predict ipv
                output_ipv = tf.matmul(embedding_p_1 * embedding_q, self.v_1)

                # predict cart
                output_cart = tf.matmul(embedding_p_2 * embedding_q, self.v_2)

                # predict buy
                output_buy = tf.matmul(embedding_p_3 * embedding_q, self.v_3)
                ...
                # predict buy
                output_buy = tf.matmul(embedding_p_2 * embedding_q, self.v_2)
...
if __name__ == '__main__':
    ...
    model = Our_Algorithm_Rev(num_users=10000, num_items=10000, args=args)
    model.build_graph()
    print(model)
    print('all done!')
