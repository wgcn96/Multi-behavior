...
class GMF_controlled:
    def __init__(self, num_users, num_items, args, use_pretrain, load='ipv'):
        ...
        self.gamma_bilinear = regs[1]
        self.loss_func = args.loss_func
        self.frozen_ui = args.frozen  # TODO(wgcn96): 层冻结
    ...
    def _create_inference(self, item_input):
        with tf.name_scope("inference"):
            ...
            self.embedding_q = tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_Q, item_input),
                                             1)  # (b, embedding_size)
            return tf.sigmoid(
                tf.matmul(self.embedding_p * self.embedding_q, self.h),
                name='output')  # (b, embedding_size) * (embedding_size, 1)
...
class pure_MLP:
    ...
    def _create_inference(self, item_input):
        with tf.name_scope("inference"):
            ...
            if self.layer_num == 0:
                return tf.sigmoid(tf.matmul(self.concat_vec, self.h),
                                  name='output')  # (b, embedding_size) * (embedding_size, 1)
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):
            ...
            self.embedding_Q = tf.Variable(
                ...
                name='embedding_Q', dtype=tf.float32)  # (items, embedding_size)
            self.h = tf.Variable(
                tf.random_uniform([3 * int(self.embedding_size / (2 ** self.layer_num)), 1],
                                  minval=-tf.sqrt(1 / self.embedding_size),
                                  maxval=tf.sqrt(1 / self.embedding_size)), name='h')
        ...
        with tf.name_scope("FC"):
            ...
            if self.layer_num == 0:
                ...
                self.W_FC = tf.Variable(
                    tf.random_uniform(shape=[3 * self.embedding_size, int(3 * self.embedding_size / 2)],
                                      minval=-tf.sqrt(1 / self.embedding_size),
...
class Multi_GMF():
    ...
    def _create_variables(self):
        with tf.name_scope('embedding'):
            ...
            # [E, 1]
            self.v_1 = tf.Variable(
                tf.random_uniform([self.embedding_size, 1],
                                  minval=-tf.sqrt(3 / self.embedding_size), maxval=tf.sqrt(3 / self.embedding_size),
                                  name='v_1', dtype=tf.float32))
            self.v_2 = tf.Variable(
                tf.random_uniform([self.embedding_size, 1],
                                  minval=-tf.sqrt(3 / self.embedding_size), maxval=tf.sqrt(3 / self.embedding_size),
                                  name='v_2', dtype=tf.float32))
            self.v_3 = tf.Variable(
                tf.random_uniform([self.embedding_size, 1],
                                  minval=-tf.sqrt(3 / self.embedding_size), maxval=tf.sqrt(3 / self.embedding_size),
                                  name='v_3', dtype=tf.float32))
    ...
    def _create_inference(self):
        with tf.name_scope('inference'):
            ...
            if self.b_num == 3:
                # predict ipv
                output_ipv = tf.matmul(tf.nn.relu(tf.matmul(z, self.W1)), self.v_1) + b
                # predict cart
                temp_cart = tf.matmul(tf.nn.relu(tf.matmul(z, self.W2)), self.v_2)
                output_cart = (temp_cart + output_ipv) / 2 + b
                # predict buy
                temp_buy = tf.matmul(tf.nn.relu(tf.matmul(z, self.W3)), self.v_3)
                ...
                if self.b_2_type == 'cb':
                    # predict cart
                    output_cart = tf.matmul(tf.nn.relu(tf.matmul(z, self.W1)), self.v_1) + b
                    # predict buy
                    temp_buy = tf.matmul(tf.nn.relu(tf.matmul(z, self.W2)), self.v_2)
                    output_buy = (temp_buy + output_cart) / 2 + b
...
class Multi_MLP():
    ...
    def _create_variables(self):
        ...
        with tf.name_scope('W_b_h'):
            # h-vector
            v_size = 2 * int(self.embedding_size / (2 ** self.layer_num))
            self.v_1 = tf.Variable(
                tf.random_uniform([v_size, 1], minval=-tf.sqrt(3 / v_size),
    ...
    def _create_inference(self):
        ...
        with tf.name_scope('inference'):
            ...
            if self.layer_num == 0:
                ...
                if self.b_num == 3:
                    # predict ipv
                    output_ipv = tf.matmul(tf.nn.relu(tf.matmul(z, self.W1) + self.b1), self.v_1) + b
                    # predict cart
                    temp_cart = tf.matmul(tf.nn.relu(tf.matmul(z, self.W2) + self.b2), self.v_2)
                    output_cart = (temp_cart + output_ipv) / 2 + b
                    # predict buy
                    temp_buy = tf.matmul(tf.nn.relu(tf.matmul(z, self.W3) + self.b3), self.v_3)
                    output_buy = (temp_buy + output_cart) / 2 + b
                    ...
                    if self.b_2_type == 'cb':
                        # predict cart
                        output_cart = tf.matmul(tf.nn.relu(tf.matmul(z, self.W1) + self.b1), self.v_1) + b
                        # predict buy
                        temp_buy = tf.matmul(tf.nn.relu(tf.matmul(z, self.W2) + self.b2), self.v_2)
                        output_buy = (temp_buy + output_cart) / 2 + b
                ...
                if self.b_num == 3:
                    # predict ipv
                    output_ipv = tf.matmul(fc_1[i], self.v_1) + b
                    # predict cart
                    temp_cart = tf.matmul(fc_2[i], self.v_2)
                    output_cart = (temp_cart + output_ipv) / 2 + b
                    # predict buy
                    temp_buy = tf.matmul(fc_3[i], self.v_3)
                    output_buy = (temp_buy + output_cart) / 2 + b
                    ...
                    if self.b_2_type == 'cb':
                        # predict cart
                        output_cart = tf.matmul(fc_1[i], self.v_1) + b
                        # predict buy
                        temp_buy = tf.matmul(fc_2[i], self.v_2)
                        output_buy = (temp_buy + output_cart) / 2 + b
...
class Our_Algorithm():
    ...
    def _create_variables(self):
        ...
        with tf.name_scope('multi_task_learning'):
            self.H = tf.Variable(tf.ones(shape=[self.num_users, 3]),
        ...
        with tf.name_scope('loss'):
            if self.loss_func == 'logloss':
                loss_em = self.lambda_bilinear * tf.reduce_sum(tf.square(self.embedding_P_1)) + \
                          ...
                          self.lambda_bilinear * tf.reduce_sum(tf.square(self.embedding_P_3)) + \
                          self.gamma_bilinear * tf.reduce_sum(tf.square(self.embedding_Q))
                loss_W1 = 0
                loss_W2 = 0
                loss_W3 = 0
...
class BPR:
    ...
    def _create_inference(self, item_input, name):
        with tf.name_scope("inference"):
            embedding_p = tf.nn.dropout(tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_P, self.user_input), 1),
                                        self.dropout)
            embedding_q = tf.nn.dropout(tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_Q, item_input), 1),
                                        self.dropout)
            # (b, embedding_size)
...
class NCF_FC:
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):
            ...
            self.v_1 = tf.Variable(tf.random_uniform([self.embedding_size * 3, 1],
                                                     minval=-tf.sqrt(1 / self.embedding_size),
                                                     maxval=tf.sqrt(1 / self.embedding_size)),
                                   name='v_1', dtype=tf.float32)

            self.v_2 = tf.Variable(tf.random_uniform([self.embedding_size * 3, 1],
                                                     minval=-tf.sqrt(1 / self.embedding_size),
                                                     maxval=tf.sqrt(1 / self.embedding_size)),
                                   name='v_2', dtype=tf.float32)

            self.v_3 = tf.Variable(tf.random_uniform([self.embedding_size * 3, 1],
                                                     minval=-tf.sqrt(1 / self.embedding_size),
                                                     maxval=tf.sqrt(1 / self.embedding_size)),
                                   name='v_3', dtype=tf.float32)
    ...
    def _create_inference(self):
        with tf.name_scope("inference"):
            embedding_p = tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_P, self.user_input), 1)
            embedding_q = tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_Q, self.item_input),
                                        1)  # (b, embedding_size)

            z = tf.concat([embedding_p, embedding_q, embedding_p * embedding_q], 1)

            output1 = tf.sigmoid(tf.matmul(z, self.v_1), name='output1')

            fc1 = tf.nn.relu(tf.matmul(z, self.W1) + self.b1)
            output2 = tf.sigmoid(tf.matmul(fc1, self.v_2), name='output2')

            fc2 = tf.nn.relu(tf.matmul(fc1, self.W2) + self.b2)
            output3 = tf.sigmoid(tf.matmul(fc2, self.v_3), name='output3')
...
class GMF_FC:
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):
            ...
            self.v_1 = tf.Variable(tf.random_uniform([self.embedding_size, 1],
                                                     minval=-tf.sqrt(3 / self.embedding_size),
                                                     maxval=tf.sqrt(3 / self.embedding_size)),
                                   name='v_1', dtype=tf.float32)

            self.v_2 = tf.Variable(tf.random_uniform([self.embedding_size, 1],
                                                     minval=-tf.sqrt(3 / self.embedding_size),
                                                     maxval=tf.sqrt(3 / self.embedding_size)),
                                   name='v_2', dtype=tf.float32)

            self.v_3 = tf.Variable(tf.random_uniform([self.embedding_size, 1],
                                                     minval=-tf.sqrt(3 / self.embedding_size),
                                                     maxval=tf.sqrt(3 / self.embedding_size)),
                                   name='v_3', dtype=tf.float32)
            # Add multi-layer
