...
def parse_args():
    ...
    parser.add_argument('--layer_num', type=int, nargs='?', default=1,
                        help='layer number')
    parser.add_argument('--regs', nargs='?', default='[5e-6,5e-6]',
...
if __name__ == '__main__':
    ...
    # batchsize = [512, 1024, 2048]   # 2048 最优 不过没有特别大变化
    layernum = [2, 2]
    ...
    for layer_num in layernum:
        ...
        if args.model == 'Our_WtNCF':
            model = Our_Algorithm_WtNCF(dataset_all[0].num_users, dataset_all[0].num_items, args)
        elif args.model == 'Our_WtAtt':
            model = Our_Algorithm_WtAtt(dataset_all[0].num_users, dataset_all[0].num_items, args)
        elif args.model == 'Our_WtFuse':
            model = Our_Algorithm_WtFuse(dataset_all[0].num_users, dataset_all[0].num_items, args)
