...
class Multi_MLP():
    ...
    def _create_inference(self):
        ...
        with tf.name_scope('inference'):
            ...
            z = tf.concat([self.embedding_p, self.embedding_q], 1, name='z')
...
class Multi_NCF:
    ...
    def _create_inference(self):
        ...
        with tf.name_scope('inference'):
            ...
            # [B, 3E]
            z = tf.concat([self.embedding_p, self.embedding_q, self.embedding_p * self.embedding_q], 1, name='z')
            ...
            if self.layer_num == 0:
                ...
                if self.b_num == 3:
                    ...
                    if self.b_2_type == 'cb':
                        ...
                        # predict cart
                        temp_cart = tf.matmul(fc_2[i], self.h_2)
                        output_cart = (temp_cart + output_ipv) / 2 + b  # TODO(wgcn96): 这里原来是temp_buy，怀疑是作者笔误
...
class BPR:
    ...
    def _create_inference(self, item_input, name):
        with tf.name_scope("inference"):
            self.embedding_p = tf.nn.dropout(tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_P, self.user_input), 1), self.dropout)
            self.embedding_q = tf.nn.dropout(tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_Q, item_input), 1), self.dropout)  # (b, embedding_size)
            return tf.matmul(self.embedding_p * self.embedding_q, self.h, name=name)  # (b, embedding_size) * (embedding_size, 1)
