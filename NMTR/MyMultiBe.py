...
def parse_args():
    parser = argparse.ArgumentParser(description="Multi-Behavior Recommendation")
    parser.add_argument('--dataset', nargs='?', default='ali',
                        help='Choose a dataset: beibei, ali')
    parser.add_argument('--model', nargs='?', default='Multi_MLP',
                        help='Choose model: GMF, MLP, FISM, Multi_GMF, Multi_BPR, BPR, CMF')
    ...
    parser.add_argument('--lr', type=float, default=0.01,
                        help='Learning rate.')
    parser.add_argument('--process_name', nargs='?', default='recover@wangchen',
...
def eval_from_saved_model(model, args):
    ...
    with tf.Session(graph=eval_graph, config=config) as sess:
        ...
        print("Final HR = %.4f, NDCG = %.4f , total time %.f" % (hr, ndcg, time()-eval_begin))
...
def training_batch_3(model, sess, batches, args, optimizer):
    ...
    if 'BPR' in args.model:
        ...
        user_input, item_input, labels = batches
        # print("check list {}, arg1: {}, arg2: {}, arg3: {}".format(len(batches), len(user_input), len(item_input), len(labels)))
        num_batch = len(batches[1])
...
if __name__ == '__main__':
    ...
    if args.model == 'Multi_GMF':
        ...
        model = CMF(dataset_all[0].num_users, dataset_all[0].num_items, args)
        print('num_users:%d   num_items:%d' % (dataset_ipv.num_users, dataset_ipv.num_items))
        model.build_graph()
        dataset = dataset_all
        print('start single process')
